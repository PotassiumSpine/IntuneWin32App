            ...
            if ($IntuneWinXMLMetaData -ne $null) {
                ...
                if ($PSBoundParameters["ScopeTagName"]) {
                    ...
                    foreach ($ScopeTagItem in $ScopeTagName) {
                        ...
                        $ScopeTag = (Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/beta/deviceManagement/getRoleScopeTagsByResource(resource='MobileApps')?`$filter=displayName eq '$($ScopeTagItem)'" -Method "GET" -ErrorAction "Stop").value
                        if ($ScopeTag -ne $null) {
                ...
                if ($PSBoundParameters["CategoryName"]) {
                    ...
                    foreach ($CategoryNameItem in $CategoryName) {
                        ...
                        $Category = (Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/beta/deviceAppManagement/mobileAppCategories?`$filter=displayName eq '$([System.Web.HttpUtility]::UrlEncode($CategoryNameItem))'" -Method "GET" -ErrorAction "Stop").value
                        if ($Category -ne $null) {
                ...
                $Win32MobileAppRequest = Invoke-MgGraphRequest  -Method "POST" -Uri "https://graph.microsoft.com/beta/deviceAppManagement/mobileApps" -Body ($Win32AppBody | ConvertTo-Json)
                if ($Win32MobileAppRequest.'@odata.type' -notlike "#microsoft.graph.win32LobApp") {
                    ...
                    if ($PSBoundParameters["CategoryName"]) {
                        if ($CategoryList.Count -ge 1) {
                            foreach ($CategoryItem in $CategoryList) {
                                ...
                                $Win32AppCategoryReference = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/deviceAppManagement/mobileApps/$($Win32MobileAppRequest.id)/categories/`$ref" -Method "POST" -Body ($CategoryBodyTable | ConvertTo-Json)
                            }
                        }
                    }
                    ...
                    $Win32MobileAppContentVersionRequest = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/beta/deviceAppManagement/mobileApps/$($Win32MobileAppRequest.id)/microsoft.graph.win32LobApp/contentVersions" -Method "POST" -Body "{}"
                    if ([string]::IsNullOrEmpty($Win32MobileAppContentVersionRequest.id)) {
                        ...
                        if ($IntuneWinFilePath -ne $null) {
                            ...
                            $Win32MobileAppFileContentRequest = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/beta/deviceAppManagement/mobileApps/$($Win32MobileAppRequest.id)/microsoft.graph.win32LobApp/contentVersions/$($Win32MobileAppContentVersionRequest.id)/files" -Method "POST" -Body ($Win32AppFileBody | ConvertTo-Json)
                            if ([string]::IsNullOrEmpty($Win32MobileAppFileContentRequest.id)) {
                                ...
                                $Win32AppFileCommitRequest = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/beta/$CommitResource" -Method "POST" -Body ($IntuneWinFileEncryptionInfo | ConvertTo-Json)
                                ...
                                switch ($CommitFileRequest.uploadState) {
                                    ...
                                    default {
                                        ...
                                        $Win32AppFileCommitBodyRequest = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/beta/deviceAppManagement/mobileApps/$($Win32MobileAppRequest.id)" -Method "PATCH" -Body ($Win32AppFileCommitBody | ConvertTo-Json)
                                        ...
                                        $Win32MobileAppRequest = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/beta/deviceAppManagement/mobileApps/$($Win32MobileAppRequest.id)" -Method "GET"
